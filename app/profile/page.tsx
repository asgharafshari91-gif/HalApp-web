      ...
      const ilceler = Array.isArray(districts)
        ? districts
            .map((d: any) => {
              ...
              const mahalleler = d.mahalleler ?? d.neighborhoods ?? d.quarters ?? d.list ?? d.items ?? [];
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (neighborhood && !neighborhoodOptions.some((x) => normKeyTR(x) === normKeyTR(neighborhood))) setNeighborhood("");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [district, locs]);
      ...
      <div className="rounded-[28px] border border-black/10 bg-white/80 p-6 shadow-[0_18px_60px_rgba(0,0,0,0.08)] dark:border-white/10 dark:bg-white/[0.04] dark:shadow-[0_18px_70px_rgba(0,0,0,0.55)]">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {/* ✅ SADECE KENDİ PROFİLİNDE DÜZENLE */}
            {isMyProfile ? (
              !editMode ? (
                <button
                  onClick={startEdit}
