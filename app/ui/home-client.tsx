...
function StatCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-[30px] border border-black/10 bg-white/75 p-5 shadow-[0_20px_70px_rgba(0,0,0,0.06)] backdrop-blur-xl transition hover:-translate-y-1 hover:shadow-[0_30px_90px_rgba(16,185,129,.18)] dark:border-white/10 dark:bg-white/[0.055]">
      ...
      <div className="relative text-[30px] font-black leading-none tracking-tight text-zinc-950 dark:text-white">
        {value}
      </div>
...
function ExpandCard({
  ...
  return (
    <button
      type="button"
      onClick={() => setOpen((v) => !v)}
      className="group relative w-full overflow-hidden rounded-[34px] border border-black/10 bg-white/75 p-6 text-left shadow-[0_24px_90px_rgba(0,0,0,0.065)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-emerald-500/30 hover:shadow-[0_34px_120px_rgba(34,197,94,.16)] dark:border-white/10 dark:bg-white/[0.045]"
    >
      ...
      <div className="relative">
        <div className="flex h-13 w-13 items-center justify-center rounded-2xl bg-emerald-500/12 text-2xl ring-1 ring-emerald-500/20 transition-transform duration-300 group-hover:scale-110">
          {icon}
        </div>
...
function PhoneMockup3D({ items }: { items: MarketItem[] }) {
  const list = items.slice(0, 8);

  return (
    <div className="relative mx-auto flex min-h-[720px] items-center justify-center overflow-visible">
      <div className="pointer-events-none absolute h-[680px] w-[680px] rounded-full bg-[radial-gradient(circle,rgba(16,185,129,.22),rgba(209,250,229,.18),transparent_68%)] blur-[18px]" />
      <div className="pointer-events-none absolute h-[520px] w-[520px] rounded-full bg-emerald-300/18 blur-[110px]" />
      ...
      <div className="group relative [perspective:1800px]">
        <div className="relative rotate-[-7deg] transition-all duration-700 [transform-style:preserve-3d] group-hover:rotate-0 group-hover:scale-[1.035]">
          <div className="pointer-events-none absolute -inset-8 rounded-[76px] bg-gradient-to-br from-emerald-300/55 via-white/20 to-transparent blur-3xl" />
                      ...
                      {[...list, ...list].map((item, i) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-[20px] bg-emerald-50 text-3xl transition group-hover/card:scale-110">
                              {item.coverUrl ? (
                                <img
                                  src={item.coverUrl}
                                  alt={item.title}
...
function PriceMovementSection({ items }: { items: MarketItem[] }) {
  const rows = useMemo(() => {
    ...
    if (!list.length) {
      return [
        { id: "demo-1", emoji: "🍋", title: "Limon", location: "Antalya / Serik", price: "Canlı", href: "/pazar" },
        { id: "demo-2", emoji: "🥑", title: "Avokado", location: "Antalya / Finike", price: "Canlı", href: "/pazar" },
        { id: "demo-3", emoji: "🍅", title: "Domates", location: "Mersin", price: "Canlı", href: "/pazar" },
        { id: "demo-4", emoji: "🍑", title: "Kayısı", location: "Malatya", price: "Canlı", href: "/pazar" },
      ];
    }
  ...
  return (
    <section className="mt-16">
      <div className="relative overflow-hidden rounded-[46px] border border-black/10 bg-white/78 p-6 shadow-[0_30px_120px_rgba(0,0,0,0.09)] backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.045] sm:p-10">
  ...
  return (
    <main className="relative w-full max-w-full overflow-x-clip">
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 sm:py-14 lg:px-8">
        <section className="relative overflow-hidden rounded-[42px] border border-black/10 bg-white/78 p-5 shadow-[0_38px_150px_rgba(0,0,0,0.10)] backdrop-blur-2xl dark:border-white/10 dark:bg-white/[0.045] sm:p-10 lg:p-12">
          ...
          <div className="relative grid gap-12 lg:grid-cols-12 lg:items-center">
            <div className="lg:col-span-6">
              ...
              <h1 className="mt-7 max-w-5xl text-[38px] font-black leading-[1.02] tracking-[-0.045em] text-zinc-950 dark:text-white sm:text-6xl lg:text-[70px]">
                Hal ticaretinde{" "}
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                <a
                  href="/pazar"
                  className="inline-flex h-14 items-center justify-center rounded-2xl bg-emerald-500 px-7 text-sm font-black text-black shadow-[0_28px_80px_rgba(34,197,94,.32)] transition hover:scale-[1.02] hover:bg-emerald-400"
                >
                  Canlı Pazarı Aç →
                </a>
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-2">
                <StatCard
                  value="81"
                  label="İl Kapsamı"
                  text="Türkiye genelinde canlı pazar hareketi."
                />
                <StatCard
                  value="Canlı"
                  label="İlan Akışı"
                  text="Yeni ürün ve talepler anlık görünür."
                />
                <StatCard
                  value="Güven"
                  label="Satıcı Profili"
                  text="Daha net, daha takip edilebilir ticaret."
                />
                <StatCard
                  value="Hızlı"
                  label="Bağlantı"
                  text="Alıcı ve satıcıya saniyeler içinde ulaş."
                />
              </div>
            </div>

            <div className="lg:col-span-6">
              <PhoneMockup3D items={marketItems} />
            </div>
          </div>
        </section>
        ...
        <section className="mt-16 overflow-hidden rounded-[42px] border border-emerald-500/15 bg-gradient-to-br from-emerald-950 via-zinc-950 to-slate-950 p-6 text-white shadow-[0_34px_140px_rgba(0,0,0,.22)] sm:p-10">
