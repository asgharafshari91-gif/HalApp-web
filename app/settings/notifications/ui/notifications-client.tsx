    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="text-sm font-black text-black/90 dark:text-white/90">{title}</div>
        ...
        <button
          ...
          className={clsx(
            ...
            value ? "bg-emerald-500" : "bg-black/10 dark:bg-white/10",
...
export default function NotificationsClient() {
  ...
  async function load() {
    ...
    try {
      ...
      // ✅ tablo yoksa bile sayfa patlamasın: "maybeSingle" + fallback
      ...
      if (error) {
        // tablo/kolon yoksa da UI çalışsın (istersen burada throw yaparsın)
        toast({ variant: "warning", title: "Uyarı", message: "Ayarlar okunamadı, varsayılanlar gösteriliyor." });
        setPrefs(DEFAULTS);
      } else {
        setPrefs({
          push_enabled: data?.push_enabled ?? DEFAULTS.push_enabled,
          message_push: data?.message_push ?? DEFAULTS.message_push,
          marketing_push: data?.marketing_push ?? DEFAULTS.marketing_push,
          email_enabled: data?.email_enabled ?? DEFAULTS.email_enabled,
          email_messages: data?.email_messages ?? DEFAULTS.email_messages,
  ...
  async function save(nextPrefs: NotifPrefs) {
    const uid = myId;
    if (!uid) return;
    setSaving(true);
    try {
      ...
      toast({ variant: "success", title: "Kaydedildi", message: "Bildirim ayarların güncellendi." });
    } catch (e: any) {
      toast({ variant: "error", title: "Kaydedilemedi", message: e?.message ?? "Hata oluştu." });
      // başarısızsa yeniden yükle
      load();
    } finally {
      setSaving(false);
    }
  ...
  function patch(p: Partial<NotifPrefs>) {
    setPrefs((prev) => {
      ...
      // ✅ ana switch kapalıysa altları da kapat (push/email)
      if (p.push_enabled === false) {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-4">
      <div className="flex items-end justify-between gap-3">
        <div>
          <div className="text-2xl font-black tracking-tight">Bildirim Ayarları</div>
          <div className="mt-1 text-sm text-black/60 dark:text-white/60">Push ve e-posta tercihlerini yönet.</div>
        </div>
      ...
      <div className="rounded-[28px] border border-black/10 bg-white/80 p-6 dark:border-white/10 dark:bg-white/[0.04]">
        {loading ? (
          ...
          <div className={clsx("space-y-3", saving && "opacity-80")}>
            <div className="text-sm font-black text-black/85 dark:text-white/85">Push Bildirimleri</div>
            ...
            <ToggleRow
              ...
              onChange={(v) => patch({ message_push: v })}
            />

            <ToggleRow
              title="Pazarlama / kampanya"
              desc="Kampanya ve duyuru bildirimleri."
              value={prefs.marketing_push}
              disabled={saving || !prefs.push_enabled}
              onChange={(v) => patch({ marketing_push: v })}
            />

            <div className="my-4 h-px bg-black/10 dark:bg-white/10" />

            <div className="text-sm font-black text-black/85 dark:text-white/85">E-posta Bildirimleri</div>
            ...
            <ToggleRow
              title="Mesaj özetleri"
              desc="Yeni mesajlarda e-posta bildirimi al."
              value={prefs.email_messages}
              disabled={saving || !prefs.email_enabled}
              onChange={(v) => patch({ email_messages: v })}
            />

            <ToggleRow
              title="Pazarlama e-postaları"
              desc="Kampanya ve duyuru e-postaları."
              value={prefs.email_marketing}
              disabled={saving || !prefs.email_enabled}
              onChange={(v) => patch({ email_marketing: v })}
            />

            <div className="mt-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 px-4 py-3 text-xs font-semibold text-black/70 dark:text-white/70">
              Not: Bildirim sistemini daha sonra FCM/OneSignal ile bağlarız. Şimdilik tercihleri DB’de tutuyoruz.
            </div>
          </div>
        )}
      </div>
