...
function initials(name?: string | null) {
  ...
  return (
    parts.map((p) => p[0]?.toUpperCase()).join("") || "HA"
  );
}
...
export default function ArchivedClient() {
  ...
  async function load() {
    ...
    try {
      ...
      const { data: ss, error: se } =
        await supabase
          .from("conversation_settings")
          .select(
            "peer_id,pinned,archived,muted"
          )
          .eq("user_id", uid)
          .eq("archived", true);
  ...
  const archived = useMemo(() => {
    ...
    const mapped = rows
      ...
      .filter(
        (
          item
        ): item is ArchivedItem =>
          item !== null
      );

    return mapped.sort((a, b) => {
      const ta = a.r.last_created_at
        ? new Date(
            a.r.last_created_at
          ).getTime()
        : 0;

      const tb = b.r.last_created_at
        ? new Date(
            b.r.last_created_at
          ).getTime()
        : 0;
