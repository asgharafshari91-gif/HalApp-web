...
export default function ArchivedClient() {
  ...
  async function load() {
    ...
    try {
      ...
      const peers = Array.from(
        new Set((ss ?? []).map((x: any) => String(x.peer_id)).filter(Boolean))
      );
      ...
      // 2) v_conversations: sadece bu peer’lar ile ilgili olanları çek
      // v_conversations yapın u1/u2 içerdiği için iki tarafta da filter lazım.
      // PostgREST OR:
      const or = peers.map((p) => `u1.eq.${p},u2.eq.${p}`).join(",");
  ...
  async function unarchive(peerId: string) {
    ...
    const cur: ConvSettings = settings[peerId] ?? { pinned: false, archived: true, muted: false };

    // optimistic UI: listeden çıkar
    setSettings((prev) => ({ ...prev, [peerId]: { ...cur, archived: false } }));
    setRows((prev) => prev.filter((r) => (r.u1 === uid ? r.u2 : r.u1) !== peerId));

    const { error } = await supabase
      .from("conversation_settings")
      .upsert(
        { user_id: uid, peer_id: peerId, pinned: cur.pinned, archived: false, muted: cur.muted },
    ...
    <div className="mx-auto w-full max-w-3xl space-y-4">
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-sm text-black/60 dark:text-white/60">Arşive aldığın sohbetler burada.</div>
        </div>
      ...
      <div className="rounded-[28px] border border-black/10 bg-white/80 p-4 dark:border-white/10 dark:bg-white/[0.04]">
        {loading ? (
          ...
          <div className="p-4 text-sm text-black/60 dark:text-white/60">Arşivde sohbet yok.</div>
        ) : (
