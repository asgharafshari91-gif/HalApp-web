// app/api/support/[id]/messages/route.ts
...
export async function GET(
  ...
  if (te) {
    return json({ error: te.message }, 400);
  }
  ...
  const { data, error } = await sb
    ...
    .select("id,ticket_id,sender_id,sender_role,message,read_at,created_at")
    .eq("ticket_id", id)
...
export async function POST(
  ...
  const { data: ticket, error: te } = await sb
    .from("support_tickets")
    .select("id,user_id,status,unread_admin_count")
    .eq("id", id)
    .maybeSingle();
  ...
  await sb
    ...
    .update({
      ...
      unread_admin_count: Number(ticket.unread_admin_count ?? 0) + 1,
      unread_user_count: 0,
    })
    .eq("id", id);
